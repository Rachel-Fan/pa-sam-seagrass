...
import cv2
import random
from utils import misc  # Assuming misc is in utils
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    # Check if this is the main process (only the main process should log)
    if misc.is_main_process():
        ...
        if not args.logfile:
            # Generate log file path based on output directory name
            args.logfile = os.path.join(args.output, f"{os.path.basename(args.output)}_train.txt")

        # If log file already exists, remove it to avoid appending to an old log
...
def evaluate(args, net, sam, valid_dataloaders, visualize=False, print_func=print):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 1000, logger=args.logfile, print_func=print):
            ...
            encoder_embedding = torch.cat([batched_output[i_l]['encoder_embedding'] for i_l in range(len(batched_output))], dim=0)
            image_pe = [batched_output[i_l]['image_pe'] for i_l in range(len(batched_output))]
            sparse_embeddings = [batched_output[i_l]['sparse_embeddings'] for i_l in range(len(batched_output))]
            dense_embeddings = [batched_output[i_l]['dense_embeddings'] for i_l in range(len(batched_output))]
            input_images = batched_output[0]['input_images']
        
            image_record = [batched_output[i_l]['image_record'] for i_l in range(len(batched_output))]
            ...
            iou = compute_iou(masks_sam, labels_ori)
            boundary_iou = compute_boundary_iou(masks_sam, labels_ori)
            pred = (masks_sam > 0.5).float()  # Threshold for binary mask
            ...
            if visualize:
                ...
                for ii in range(len(imgs)):
                    ...
                    # show_anns(masks_pa_vis[ii], None, labels_box[ii].cpu(), None, save_base, imgs_ii, show_iou, show_boundary_iou)
                    show_anns(masks_pa_vis[ii], None, None, None, save_base, imgs_ii, show_iou, show_boundary_iou)
            ...
            loss_dict = {
                ...
                "precision": precision,
                "recall": recall,
                "hausdorff": hausdorff if hausdorff_list else 0
            }
            loss_dict_reduced = misc.reduce_dict(loss_dict)
            metric_logger.update(**loss_dict_reduced)
        ...
        averaged_metrics = {
            ...
            "hausdorff": np.mean(hausdorff_list) if hausdorff_list else 0
        }
...
if __name__ == "__main__":
    ...
    dataset_Alaska = {"name": "Alaska",
                 "im_dir": "./data/2025/Alaska/train/image",
                 "gt_dir": "./data/2025/Alaska/train/image",
                 "im_ext": ".png",
                 "gt_ext": ".png",
                 "im_ch4_dir": "./data/2025/Alaska/train/glcm"}
    ...
    dataset_Alask_val = {"name": "Alaska",
                 ...
                 "im_ext": ".png",
                 "gt_ext": ".png",
                 "im_ch4_dir": "./data/2025/Alaska/valid/glcm"}
    ...
    dataset_Alaska_test = {"name": "Alaska",
                ...
                "im_ext": ".png",
                "gt_ext": ".png",
                "im_ch4_dir": "./data/2025/Alaska/test/glcm"}
