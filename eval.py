...
def main(train_datasets, valid_datasets, args):
    ...
    # Initialize the model (net) only when not in eval mode
    if not args.eval:
    ...
    if not args.eval:
        ...
        evaluate(args, net_without_ddp, sam, valid_dataloaders, args.visualize, print_func=print)
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        for data in metric_logger.log_every(train_dataloaders, 20, logger=args.logfile, print_func=print):
            ...
            # Here is the simplified model call
            masks_sam = net(inputs)

            loss_mask, loss_dice = loss_masks_whole(masks_sam, labels/255.0, len(masks_sam))
            loss = loss_mask + loss_dice

            loss_mask_final, loss_dice_final = loss_masks_whole_uncertain(masks_sam, masks_sam, labels/255.0, masks_sam, len(masks_sam))
            loss = loss + (loss_mask_final + loss_dice_final)
            loss_uncertain_map, gt_uncertain = loss_uncertain(masks_sam, labels)
...
def evaluate(args, net, sam, valid_dataloaders, visualize=False, print_func=print):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        biou_result = []
        img_id = []
        dataset_name = ['DIS', 'COIFT', 'HRSOD', 'ThinObject', 'dataset_train', 'dataset_valid', 'Oregon']
        total_time = 0
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 1000, logger=args.logfile, print_func=print):
            ...
            # 调用模型，传递 multimask_output 参数
            masks_sam = net(
                inputs_val,
                multimask_output=False  # 传递 multimask_output 参数
            )
            ...
            if visualize:
                ...
                for ii in range(len(inputs_val)):
                    base = data_val['imidx'][ii].item()
                    print('base:', base)
                    save_base = os.path.join(args.visualize_output, str(k) + '_' + str(base))
                    ...
                    imgs_ii = inputs_val[ii].permute(1, 2, 0).cpu().numpy().astype(np.uint8)
