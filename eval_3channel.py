...
class SimpleResizeDataset(Dataset):
    ...
    - Resizes both to input_size (H, W) using bilinear (image) / bilinear (gt, then threshold later)
    - Returns tensors ready for model (float32, CHW)
    """
    def __init__(self, pairs: List[Tuple[str, str]], input_size=(512, 512)):
        self.pairs = pairs
    ...
    def __getitem__(self, idx):
        ...
        im = io.imread(ip)  # H, W, C (3) expected
        ...
        gt_r = F.interpolate(gt_t, size=(self.H, self.W), mode="bilinear", align_corners=False).squeeze(0)  # 1,H,W

        sample = {
            "image": im_r,                   # 3,H,W float32
            "label": gt_r,                   # 1,H,W float32 (not binarized yet)
            "name": os.path.splitext(os.path.basename(ip))[0],
            "im_path": ip,
            "gt_path": gp
        }
...
# ---------------------------
def parse_args():
    p = argparse.ArgumentParser("PA-SAM eval (RGB only) — export 512x512 BW masks + metrics", add_help=True)
...
def main():
    ...
    # Dataset / DataLoader
    ds = SimpleResizeDataset(pairs, input_size=args.input_size)
